...
local function GetScriptDir()
    ...
    if StartsWith(src, "@") then src = src:sub(2) end
    -- return directory with trailing slash/backslash (or "./")
    return src:match("^(.*[\\/])") or "./"
end

local function ReadModsFromConfig()
    local scriptDir = GetScriptDir()
    local cfgPath = scriptDir .. "config.txt"
    local lines = LinesFrom(cfgPath)  -- ignores lines starting with ';'
    local mods = {}
...
local function LoadModConfigs()
    ...
    -- 1) Read mod names from config.txt (same folder as this lua)
    local modNames = ReadModsFromConfig()
    if #modNames == 0 then
        Log("No mods listed in config.txt next to this script. Nothing to load.\n")
    end

    -- 2) For each mod name, create a default config (same defaults you already use)
    for _, ModName in ipairs(modNames) do
        if not Mods[ModName] then
            Mods[ModName] = {
                AssetName      = DefaultModConfig.AssetName,
                AssetNameAsFName = DefaultModConfig.AssetNameAsFName,
                AssetPath      = string.format("/Game/Mods/%s/ModActor", ModName),
                Name           = ModName
            }
            Log(string.format("Configured mod from config.txt: %s\n", ModName))
        end
    end
