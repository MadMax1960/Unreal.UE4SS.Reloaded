...
internal sealed class UE4SSSettings
{
    ...
    private static string BuildSettings(Config config, IEnumerable<string> additionalModFolderPaths)
    {
        ...
        sb.AppendLine("; Can be relative to working directory or an absolute path.");
        sb.AppendLine("; Example: ControllingModsTxt = ../MyMods/mods.txt");
        sb.AppendLine("; Default: Empty (parse mods.txt from all mod directories)");
        sb.AppendLine($"ControllingModsTxt = {config.ControllingModsTxt}");
        sb.AppendLine();
        sb.AppendLine("[General]");
        sb.AppendLine("EnableHotReloadSystem = " + BoolToNumeric(config.EnableHotReloadSystem));
        sb.AppendLine();
        sb.AppendLine("; The key that will trigger a reload of all mods.");
        sb.AppendLine("; The CTRL key is always required.");
        sb.AppendLine("; Valid values (case-insensitive): Anything from Mods/Keybinds/Scripts/main.lua");
        sb.AppendLine("; Default: R");
        sb.AppendLine($"HotReloadKey = {config.HotReloadKey}");
        sb.AppendLine();
        ...
        sb.AppendLine();
        sb.AppendLine("; Whether to perform a single AOB scan as soon as possible after the game starts.");
        sb.AppendLine("; Default: 0");
        sb.AppendLine("DoEarlyScan = " + BoolToNumeric(config.DoEarlyScan));
        sb.AppendLine();
        sb.AppendLine("; When the search query in the Live View tab is a hex number, try to find the UObject that contains that memory address.");
        sb.AppendLine("; Default: false");
        sb.AppendLine("bEnableSeachByMemoryAddress = " + BoolToTrueFalse(config.EnableSearchByMemoryAddress));
        sb.AppendLine();
        ...
        sb.AppendLine("HookEndPlay  = " + BoolToNumeric(config.HookEndPlay));
        ...
        sb.AppendLine("HookUObjectProcessEvent = " + BoolToNumeric(config.HookUObjectProcessEvent));
        sb.AppendLine("HookProcessConsoleExec = " + BoolToNumeric(config.HookProcessConsoleExec));
        sb.AppendLine("HookUStructLink = " + BoolToNumeric(config.HookUStructLink));
